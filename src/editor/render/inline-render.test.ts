...
describe("renderInlineMarkdown — inline math", () => {
  ...
  it("marks invalid KaTeX as visible math errors", () => {
    const container = document.createElement("div");
    renderInlineMarkdown(container, "$x_$");
...
describe("renderInlineMarkdown — surface policies", () => {
  it("renders links as anchors in document-inline", () => {
    ...
    expect(html).toBe(
      '<a class="cf-doc-link cf-link-rendered" href="https://example.com" data-cf-link-layout="flow">text</a>',
    );
  });
  ...
  it("marks document links as atomic", () => {
    ...
    expect(html).toBe(
      '<a class="cf-doc-link cf-link-rendered" href="chapter.md" data-cf-link-layout="atomic">text</a>',
    );
  });
