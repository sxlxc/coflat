...
function normalizeCslItem(item: CslJsonItem): CslJsonItem {
  const rawCitationKey = item["citation-key"];
  const citationKey = typeof rawCitationKey === "string"
    ? rawCitationKey
    : undefined;
...
export function parseBibTeXResult(content: string): BibTexParseResult {
  ...
  try {
    ...
    const result = { items: cite.data.map(normalizeCslItem) };
    ...
    if (sanitized !== content) {
      try {
        ...
        const result = {
          items: cite.data.map(normalizeCslItem),
          recovered: true,
        };
        cacheBibParseResult(content, result);
        return result;
